...
class FooterEventHandler implements IEventHandler {
    ...
    private final Table tableFooter;
    private final float tableHeight;
    private final Document doc;

    public FooterEventHandler(Document doc, LayoutGrid grid) {
        this.doc = doc;
        tableFooter = ReportExporterPDF.createTable(grid);
        tableHeight = getTableHeight(tableFooter);
    }
    ...
    public void handleEvent(Event currentEvent) {
        ...
        float coordY = pageSize.getBottom() + 10;
        float width = pageSize.getWidth() - doc.getRightMargin() - doc.getLeftMargin();
        float height = getTableHeight();
        Rectangle rect = new Rectangle(coordX, coordY, width, height);
        new Canvas(canvas, pdfDoc, rect).add(tableFooter).close();
    }
